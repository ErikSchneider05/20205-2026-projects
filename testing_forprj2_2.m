%%This code is identical to part 2 just the curve had data points outside
%%by eleminating any points on top fo the curve then i can possible get a
%%better answer then 2.25


%% Input data
    x = [1.0; 2.0; 3.0; 1.5; 2.5; 3.5; 2.25];
    y = [22.14; 21.70; 22.08; 21.96; 21.79; 22.27; 21.68];
    x1 = x([1, 2, 6, 7]); %first attempt r sqaurded of 0.997 and 2.172
    %inches is lap time 21.684 seconds 
    y1 = y([1, 2, 6, 7]);
    %x1 = x([1, 2, 7]);    % r sqaured is 1 and 2.264 inches at 21.68 
    %y1 = y([1, 2, 7]);
    %x1 = x([2, 7]);
    %y1 = y([2, 7]);
%% Vandermonde matrix 
...
%% regression fit math
    y_hat = Beta(1) + Beta(2) * x1 + Beta(3) * x1.^2;
    y_avg = mean(y1);
    n = length(y1);
    p = 2;
    %%=========
    ss_res = sum((y1 - y_hat).^ 2);
    ss_t = sum((y1 - y_avg) .^2);
    %%=========
